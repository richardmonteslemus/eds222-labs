...
```{r}
df = table(penguins$species, penguins$island)
df
```
...
How would you use `count()`, `group_by()`, `mutate()`, and pivoting to create row- and column-wise proportion tables?
...
A histogram is only used for counts along the y axis, but barplots don't have to.

For a histogram the x axis contains a continuous value, for a bar plot it is discrete.
...
```{r}
salton_sea_bore <- read_csv(here("week1", "SaltonSea_WellLogDatabase_Borehole.csv"))
```

```{r}
salton_sea_bore %>% 
  ggplot(aes(Hole_Depth)) +
  geom_histogram(fill = c("blue")) + 
  labs(x = "Hole Depth (m)", title = "Salton Sea Borehole Depths") + 
