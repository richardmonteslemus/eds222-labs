...
H0: There is no difference in outcome between the garden staple and popsicle stick method.

HA: The popsicle stick method is more successful than the garden staple method
...
success_props <- restoration %>% 
  group_by(treatment) %>% 
  summarize(prop_success = sum(success)/sum(n()))
...
p_hat <- (sum(restoration$success)/ length(restoration$success))
...
obs_table <- restoration %>% 
  group_by(treatment) %>% 
  summarise(number_obs = sum(n()))

n1 <- obs_table$number_obs[1]
n2 <- obs_table$number_obs[2]
...
Its greater than the critical threshold, therefore, we do not have sufficient evidence to reject the null hypothesis.
...
We are 95% confident the true difference in proportions falls between -0.05629908 and 0.33963242
...
The sampling distribution of the statistics assuming the null hypothesis is true.

**Q12: What are bootstrapping and mathematical models [both]{.underline} trying to estimate for confidence intervals?**

They are trying to estimate the sampling distribution of the statistic assuming the difference is real.
...
Why do we use a pooled phat?

-   The null hypothesis assumes there is no difference between popsicle and garden staples. So we remove the category.

-   rbinom

    -   when do we use for resampling?

        -   we dont, rbinom just makes brand new data so this is good for simulations, but not for real tests. and its a yes or no success.
...
-   Inverse of pnorm
    ...
    -   say what you want area to be and this gives you where on the x axis these values fall.
...
# Whats the probability of getting a random number LESS 
# than -0.6
...
# Associte CIS with qnorm() applied to the sampling distribution (not under the H0)
